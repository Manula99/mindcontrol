...
addNewDrawing = function (template) {
  ...
  var entry = {
    contours: [],
    checkedBy: Meteor.user().username,
    name: 'Drawing ' + contours.length,
    uuid: guid(),
  };
  contours.push(entry);
...
var addPapaya = function (data, entry_type, template_instance, callback) {
  ...
  var res = _.find(Meteor.settings.public.modules, function (x) {
    return x.entry_type == entry_type;
  });
  ...
  for (i = 0; i < data.check_masks.length; i++) {
    //never load more than nOverlays images
    // var url = res["staticURL"]+data["check_masks"][i]
    var url = data['check_masks'][i];
...
var load_hotkeys = function (template_instance) {
  ...
  contextHotkeys.add({
    ...
    callback: function () {
      ...
      if (!isHidden) {
        papaya.Container.hideImage(0, idx);
      } else {
...
var sync_templates_decorator = function (template_instance) {
  return function (data) {
    ...
    if (data['action'] == 'update') {
      for (var key in data['data']) {
        ...
        var current = template_instance[key].get();
        //console.log("current is", current)
        var entry = _.find(current, function (e) {
          return e.uuid == data['data'][key]['uuid'];
        });
...
Template.view_images.onCreated(function () {
  ...
  var qc = Session.get('currentQC');
  var settings = _.find(Meteor.settings.public.modules, function (x) {
    return x.entry_type == qc.entry_type;
  });
  ...
  if (settings.usePeerJS) {
    ...
    peer.on('open', function () {
      ...
      var current_profile = Meteor.users.findOne({ _id: Meteor.userId() })
        .profile;
      if (!current_profile) {
        current_profile = {};
      }
      ...
      console.log(
        'profile si',
        Meteor.users.findOne({ _id: Meteor.userId() }).profile
      );
    });
...
Template.view_images.helpers({
  ...
  peerUsers: function () {
    ...
    if (settings.usePeerJS) {
      ...
      var to_return = Meteor.users.find({
      ...
      if (to_return.count) {
        ...
        if (!conns.length) {
          var dude = Meteor.users.findOne({
            _id: { $in: userIds, $ne: Meteor.userId() },
          });
          if (dude) {
            console.log(
              "there are no connections but there is another person out there, so i'm connecting now",
              dude.username
            );
  ...
  onPointNote: function () {
    ...
    return template_decorator(
      Template.instance().loggedPoints,
      lp,
      idx,
      'note'
    );
  ...
  onContourNote: function () {
    ...
    if (contours) {
      ...
      if (contours[idx]) {
        ...
        var tempate_decorator2 = function (
          template_instance,
          selected,
          idx,
          contours
        ...
        return tempate_decorator2(
          Template.instance().contours,
          selected,
          idx,
          Template.instance().contours.get()
        );
      }
    }
...
Template.view_images.events({
  'submit .new-qc': function (event, template) {
    ...
    if (qc['quality_check'] && update['quality_vote'].length === 0) {
      vote_entry = {
        quality_check: qc.quality_check,
    ...
    vote_entry = {
      quality_check: update.quality_check,
  ...
  'click #screenshot': function (event, template) {
    ...
    var iframe =
      "<iframe width='100%' height='100%' style='border:0;' src='" +
      a +
      "'></iframe>";
    var x = window.open();
    x.document.open();
  ...
  'click .goto_coor': function (event, template) {
    ...
    var screenCoor = papayaContainers[0].viewer.convertCoordinateToScreen(
      this.matrix_coor
    );
  ...
  'click #paintPicker': function (event, template) {
    ...
    var offset = ori.convertIndexToOffset(
      papayaRoundFast(c.x),
      papayaRoundFast(c.y),
      papayaRoundFast(c.z)
    );
  ...
  'click #loadPainter': function (event, template) {
    var qc = Session.get('currentQC');
    var output = Subjects.findOne(
      { entry_type: qc.entry_type, name: qc.name },
      { check_masks: 1, _id: 0, name: 1, loggedPoints: 1, contours: 1 }
    );
    if (output) {
  ...
  return function () {
    ...
    output.painters.forEach(function (val, idx, arr) {
      ...
      val.matrix_coor.forEach(function (val2, idx2, arr2) {
        setValue(
          papayaRoundFast(val2.x),
          papayaRoundFast(val2.y),
          papayaRoundFast(val2.z),
          paintVal
        );
      });
